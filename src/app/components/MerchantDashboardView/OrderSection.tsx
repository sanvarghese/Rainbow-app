                        ...
                        <div className="flex flex-col gap-1 ml-auto">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <input
                              ...
                              defaultValue={order.deliveryDate ? (typeof order.deliveryDate === 'string' ? order.deliveryDate.split('T')[0] : new Date(order.deliveryDate).toISOString().split('T')[0]) : ''}
                              onChange={(e) => e.target.value && updateDeliveryDate(order._id, e.target.value)}
                              disabled={updatingOrder === order._id}
                              className="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 text-sm"
                            />
                          </div>
                          {order.deliveryDate && (
                            <p className="text-xs text-gray-500">Current: {formatDate(order.deliveryDate)}</p>
                          )}
                        </div>
                      </div>
                    </div>
